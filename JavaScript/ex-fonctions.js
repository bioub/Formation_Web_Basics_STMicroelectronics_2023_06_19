...
function dejaSaisis(nbs = []) {
  // aujourd'hui ne pas écrire un test qui viendrait
  // tester un type, ex : est-ce que nbs est un tableau ?
  // on a un langage pour ça : TypeScript
  if (!nbs.length) { // Fail-fast, avoid else keyword
    return undefined;
  }
...
// Version imperative (avec boucle + if)
...
// Version fonctionnelle (avec des fonctions)
function totalPairs(nbs = []) {
